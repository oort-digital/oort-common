...
export const Menu = ({ title, href, subMenuClassName, subMenuItemClassName, subMenu = [], maxDescriptionLen, subMenuStyle }: IMenuProps) => {

    const router = useRouter();

    const isActive = subMenu.some(x => x.href === router.pathname)
    ...
    const renderSubMenu = (subMenu: SubMenu[], maxDescriptionLen?: number) =>
        <ul className={classNames(styles.submenu, styles.hide, subMenuClassName)} style={subMenuStyle}>
            <div className={styles.arrow}>
            </div>
            {
                subMenu.map(menu => <SubMenuItem
                    key={menu.title}
                    item={menu}
...
type EventType = React.MouseEvent<HTMLUListElement> | React.AnimationEvent<HTMLUListElement>
const setClasses = (event: EventType, addClass: string, removeClass: string | string[]) => {
