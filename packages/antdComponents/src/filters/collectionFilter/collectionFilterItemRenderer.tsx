...
export const collectionItemRenderer = (
  selectMode: SelectMode,
  className: string,
  itemTitleMaxLen: number,
  selected: ISelectedParameter,
  favorite?: ISelectedParameter
) => {
  return (item: ICollectionFilterItem) => {
      ...
      if (favorite) {
        ...
        return (
          <Col>
            <FavoriteCheckBox
              checked={isFavorite}
              onChange={(checked) => favorite.onChange(item, checked)}
            />
          </Col>
        );
      }
    ...
    return (
      <Row
        ...
        align="middle"
        onClick={() => selected.onChange(item, !isSelected)}
        className={className}
      >
        <Col>
        ...
        <Col flex="auto" className={styles.item_name}>
          {collectionName}
        </Col>
        {renderFavorite()}
      </Row>
