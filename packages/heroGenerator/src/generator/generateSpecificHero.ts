...
const {
  ...
  PADDING,
  MARGIN_SKILL_TOP,
  MARGIN_FIRST_NAME_TOP,
  MARGIN_SECOND_NAME_TOP,
  MARGIN_SECOND_MARGIN_BOTTOM,
  MARGIN_SKILL_LEVEL_TOP,
  SKILL_LEVEL_HEIGHT,
...
const { PIONEER_BUILDER } = require("./clans");
...
const replaceColor = (partFileContent, targetColor) =>
  partFileContent
    .replace(
      new RegExp(hairColors[colors.default].main, "gi"),
      hairColors[targetColor].main
    )
    .replace(
      new RegExp(hairColors[colors.default].lower, "i"),
      hairColors[targetColor].lower
    )
    .replace(
      new RegExp(hairColors[colors.default].upper, "i"),
      hairColors[targetColor].upper
    );

const replaceBrandBodyColor = (partFileContent, targetColor) =>
  partFileContent.replace(new RegExp("#00000000", "gi"), targetColor);
...
const generateCharacterBodyCanvas = async (
  ...
  return drawCharacterParts(
    await Promise.all([
      loadCharacterPart(
        getBodyAsset(bodyNum, clan, customBrand),
        color,
        customBrand,
        brandColor
      ),
...
const generateCharacterImage = async (hero) => {
  ...
  const headCanvas = await generateCharacterHeadCanvas(
    ...
    hero.bodyParts.head,
    hero.bodyParts.eyes,
    hero.bodyParts.beard,
    hero.bodyParts.moustache,
    hero.bodyParts.nose,
    hero.bodyParts.brow,
    hero.hairColor
  );
  ...
  // prepare canvas
  const characterCanvas = createCanvas(
    bodyCanvas.width,
    bodyCanvas.height + headCanvas.height / 2
  );
...
export const generateSpecificWholeImage = async (hero) => {
  ...
  canvasContext.drawImage(
    characterCanvas,
    characterCanvas.width * 0.1,
    characterCanvas.height * 0.08,
    raceWidth,
    raceHeight,
    dx,
    dy,
