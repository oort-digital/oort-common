...
export class OortDasboardApi extends BaseAPI implements IOortDasboardApi {
  ...
  static createSingleton(config: IAPIConfig): OortDasboardApi {
    if (!OortDasboardApi._singleton) {
      OortDasboardApi._singleton = new OortDasboardApi(
        "OortDasboardApi",
        config,
      );
    }
  ...
  dashboard = async (signal: AbortSignal): Promise<IDasboardResponse> => {
    const response: AxiosResponse<IDasboardResponse> = await this._axios.get<
      IDasboardResponse,
      AxiosResponse<IDasboardResponse>
  ...
  nfts = async (
    ...
    signal: AbortSignal,
  ): Promise<INftsResponse> => {
    const urlParams = new URLSearchParams([["filter-games", `${gamesOnly}`]]);
    ...
    if (
      twitterFollowersRange &&
      twitterFollowersRange[0] &&
      twitterFollowersRange[1]
    ) {
      urlParams.append(
        "number-filter-starts",
        twitterFollowersRange[0].toString(),
      );
      urlParams.append(
        "number-filter-ends",
        twitterFollowersRange[1].toString(),
      );
    ...
    if (
      discordFollowersRange &&
      discordFollowersRange[0] &&
      discordFollowersRange[1]
    ) {
      urlParams.append(
        "number-filter-starts",
        discordFollowersRange[0].toString(),
      );
      urlParams.append(
        "number-filter-ends",
        discordFollowersRange[1].toString(),
      );
    ...
    const response: AxiosResponse<INftsResponse> = await this._axios.get<
      INftsResponse,
      AxiosResponse<INftsResponse>
    >(`/game-hub/nft-collections`, { params: urlParams, signal: signal });
  ...
  games = async (
    ...
    const response: AxiosResponse<IGamesResponse> = await this._axios.get<
      IGamesResponse,
      AxiosResponse<IGamesResponse>
    >(`/game-hub/games`, { params: urlParams, signal: signal });
  ...
  gameDetail = async (
    { gameUri }: IGameDetailParams,
    signal: AbortSignal,
  ): Promise<IGameDetailResponse> => {
    const response: AxiosResponse<IGameDetailResponse> = await this._axios.get<
      IGameDetailResponse,
      AxiosResponse<IGameDetailResponse>
    >(`/game-hub/games/${gameUri}`, getConfig(false, signal, undefined));

    return response.data;
  };

  nftDetail = async (
    { nftCollectionId }: INftDetailParams,
    signal: AbortSignal,
  ): Promise<INftDetailResponse> => {
    const response: AxiosResponse<INftDetailResponse> = await this._axios.get<
      INftDetailResponse,
      AxiosResponse<INftDetailResponse>
    >(`/game-hub/nft-collections/${nftCollectionId}`, getConfig(false, signal));
    return response.data;
  };

  nftDetailByAddress = async (
    { collectionAddress }: INftDetailByAddressParams,
    signal: AbortSignal,
  ): Promise<INftDetailResponse> => {
    const response: AxiosResponse<INftDetailResponse> = await this._axios.get<
      INftDetailResponse,
  ...
  getSearch = async (
    { keywords, url, pageSize, pageNum }: ISearchParams,
    signal: AbortSignal,
    ...
    const response: AxiosResponse<ISearchResultResponse> =
      await this._axios.get<
        ISearchResultResponse,
        AxiosResponse<ISearchResultResponse>
      >(`/game-hub/${url}`, { params: urlParams, signal: signal });

    return response.data;
  };

  likeGame = async (
    { gameUri, action }: ILikeGameParams,
    signal: AbortSignal,
  ): Promise<void> => {
  ...
  likeReview = async (
    { gameUri, reviewId, action }: ILikeReviewParams,
    signal: AbortSignal,
  ): Promise<void> => {
    const urlParams = new URLSearchParams([["action", `${action}`]]);
  ...
  saveReview = (
    { content, gameUri }: ISaveReviewParams,
    signal: AbortSignal,
  ): Promise<void> =>
    this._axios.put(
  ...
  saveFeedback = (
    { gameUri, isGameFun, isGameProfitable, rating }: ISaveFeedbackParams,
    signal: AbortSignal,
  ): Promise<void> => {
  ...
  getReviews = async (
    { pageNum, pageSize, gameUri }: IReviewsParams,
    signal: AbortSignal,
  ): Promise<IReviewResponse> => {
    const urlParams = this.addPagingParams({ pageSize, pageNum });

    const response: AxiosResponse<IReviewResponse> = await this._axios.get<
      IReviewResponse,
  ...
  getCurrentUserReview = async (
    gameUri: string,
    signal: AbortSignal,
  ): Promise<ICurUserReview> => {
    const response: AxiosResponse<ICurUserReview> = await this._axios.get<
      ICurUserReview,
      AxiosResponse<ICurUserReview>
    >(
