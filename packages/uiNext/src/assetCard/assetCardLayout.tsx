...
import { cutLongString, toMasked } from "@oort-digital/utils";
...
export const AssetCardLayout = <TAsset extends IAssetItem>({
  assetItem,
  owner,
  marketplace,
  chainId,
    ...
    <Row justify="space-between">
      <Col>
        <div className={styles.owner}>
          <span className={styles.lable}>Owner:</span>
          <span className={styles.address}>{toMasked(owner)}</span>
          <ViewOnButton
            nftScanConfig={nftScanConfig}
            className={styles.view_on}
            assetItem={assetItem}
            chainId={chainId}
  ...
  const assetData = (
    <div className={styles.asset_data}>
  ...
  const cardClass = showZeroCollateral
    ? `${styles.layout} ${styles.zero_collateral_layout}`
    : styles.layout;
