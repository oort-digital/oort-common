...
import {
  INftOwnerProvider,
  IFeatchNftOwnerResponse,
  ProviderKind,
  IFeatchAccountNftsParams,
  IFeatchNftImageSrc,
  IFeatchNftParams,
  IFeatchNftsResponse,
  INft,
...
export type GenerateHeroFunc = (params: {
  tokenAddress: string;
  tokenId: string;
}) => Promise<any>;
...
export class MoralisNftProviderNoSdk
  implements IAssetsProvider, IAssetProvider, INftOwnerProvider
{
  public readonly providerKind: ProviderKind = ProviderKind.Moralis;

  public readonly chainId: number;

  public async featchNftOwner(params: {
    address: string;
    tokenId: string;
    ...
    const axiosResponse = await this._axios.get<
      IMoralisPagingResponse<IRawNft[]>
    >(`/nft/${address}/${tokenId}/owners`, config);
  ...
  public async featchNft({
    address,
    tokenId,
  }: IFeatchNftParams): Promise<INft | null> {
    const config = {
      params: {
        chain: this._chainIdHex,
        format: "decimal",
    ...
    try {
      const axiosResponse = await this._axios.get<IRawNftWithNormMetadata>(
        `/nft/${address}/${tokenId}`,
        config
      );
  ...
  public async featchNftImageSrc(
    params: IFeatchNftImageSrc
  ...
  public async featchAccountNfts(
    params: IFeatchAccountNftsParams
  ): Promise<IFeatchNftsResponse> {
    ...
    const axiosResponse = await this._axios.get<
      IMoralisPagingResponse<IRawNftWithUnparsedMetadata[]>
    >(`/${ownerAddress}/nft`, config);
    const moralisResponse = axiosResponse.data;
    const promises = moralisResponse.result.map((x) =>
      this.mapNft(
        x,
        x.metadata ? (JSON.parse(x.metadata) as IMoralisMetadata) : null
      )
    );
  ...
  private async tryFetchMetadata(
    token_uri: string,
    tokenAddress: string,
    tokenId: string
  ): Promise<IMoralisMetadata | null> {
    try {
      let response = await this._axios.get(token_uri);
      if (
        response.data?.description === "Oort Hero blind box" &&
        this._generateHero
      ) {
  ...
  private ParseImage(
    ...
    this._logger.trace(
      `No image. TokenAddress: ${tokenAddress} TokenId: ${tokenId} Metadata: ${metadata}`
    );
    return undefined;
  }
  ...
  private ParseNftName(
    ...
    this._logger.trace(
      `No nftName. TokenAddress: ${tokenAddress} TokenId: ${tokenId} Metadata: ${metadata}`
    );
    return undefined;
  }
  ...
  private ParseDescription(
    ...
    this._logger.trace(
      `No description. TokenAddress: ${tokenAddress} TokenId: ${tokenId} Metadata: ${metadata}`
    );
    return undefined;
  }
  ...
  private static ParseContractType(
    tokenAddress: string,
    tokenId: string,
    rawNftType: string | null,
    logger: ILogger
  ): NftType {
    if (rawNftType === "ERC721") {
      return NftType.ERC721;
    }
    if (rawNftType === "ERC1155") {
      return NftType.ERC1155;
    }
    logger.debug(
      `Unknow contarct type: ${rawNftType}. TokenAddress: ${tokenAddress} TokenId: ${tokenId}`
    );
    return NftType.UNKNOWN;
  }

  private async mapNft(
    rawNft: IRawNftWithUnparsedMetadata,
    metadata: IMoralisMetadata | null
  ): Promise<INft> {
    ...
    if (!metadata && token_uri) {
      ...
      metadata = await this.tryFetchMetadata(
        token_uri,
        token_address,
        token_id
      );
    ...
    const result: INft = {
      ...
      contractType: MoralisNftProviderNoSdk.ParseContractType(
        token_address,
        token_id,
        rawNft.contract_type,
        this._logger
      ),
