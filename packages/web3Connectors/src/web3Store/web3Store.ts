...
import {
  ...
  IFaceWalletOptions,
} from "../connectors/faceWalletConnector";
import {
  EMPTY_CHAIN,
  IChain,
  IChainInfo,
  isChainEmpty,
} from "../publicTypesAndInterfaces";
...
export class Web3Store<TChain extends IChain> implements IWeb3Store {
  ...
  constructor(params: IWeb3StoreParams<TChain>) {
    ...
    this._connectorProvider = new ConnectorProvider(
      params.logger,
      Web3Store.createWallets(params)
    );
  ...
  private static createWallets<TChain extends IChain>({
    supportedWallets,
    supportedChains: supportedChains,
    logger,
    faceWalletCredentials,
    walletConnectProjectId,
    walletConnectModalZIndex,
  }: IWeb3StoreParams<TChain>): BaseConnector[] {
    const result: BaseConnector[] = [];

    if (
      !supportedWallets ||
      supportedWallets.some((x) => x === ConnectorNames.Injected)
    ) {
    ...
    if (
      !supportedWallets ||
      supportedWallets.some((x) => x === ConnectorNames.WalletConnect)
    ) {
      if (!walletConnectProjectId) {
        throw new Error(
          "You forgot pass IWeb3StoreParams.walletConnectProjectId"
        );
      }
      result.push(
        new WalletConnectConnector({
          ...
          modalZIndex: walletConnectModalZIndex,
        })
      );
    }
    ...
    if (
      !supportedWallets ||
      supportedWallets.some((x) => x === ConnectorNames.FaceWallet)
    ) {
      if (!faceWalletCredentials) {
        throw new Error(
          "You forgot pass IWeb3StoreParams.faceWalletCredentials"
        );
      }
  ...
  private async onConnect(connector: IConnector) {
    ...
    connector.onAccountsChanged((accounts: string[]) =>
      that.accountChangeHandler.call(that, accounts)
    );
