...
const Template: ComponentStory<typeof FakeComponent> = (_args: any) => {
  ...
  const [curConnector, setCurConnector] = useState<ConnectorNames>(
    ConnectorNames.WalletConnect
  );

  const getConnectorInstance = (): BaseConnector => {
    if (curConnector === ConnectorNames.Injected) {
      return injected;
    }
    if (curConnector === ConnectorNames.FaceWallet) {
      return faceWallet;
    }
  ...
  return (
    <div>
      <select
        value={curConnector}
        onChange={(ev: any) => setCurConnector(ev.target.value)}
      >
      ...
      <button disabled={connected} onClick={() => connect(1)}>
        Connect Ethereum
      </button>
      <button
        disabled={!connected || chainId === 137}
