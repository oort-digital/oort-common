...
export abstract class BaseConnector {
  ...
  protected accountsChangedHandler: AccountChangedHandlerType = (
  ...
  protected chainChangedHandler: ChainChangedHandlerType = (
    chainId: string
  ) => {
  ...
  protected disconnectHandler: DisconnectHandlerType = (error: any) => {
    this.logger.debug(
      `${this.name}.disconnectHandler ${JSON.stringify(error)}`
    );
