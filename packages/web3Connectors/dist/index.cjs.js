...
var faceSdk = require('@haechi-labs/face-sdk');
...
class BaseConnector {
  ...
  async getSigner() {
    const provider = new ethers.providers.Web3Provider(await this.getRawProvider());
...
const getNetworkById = (id) => {
  if (id === 1) {
    return faceSdk.Network.ETHEREUM;
  }
  if (id === 80001) {
    return faceSdk.Network.MUMBAI;
  }
  if (id === 5) {
    return faceSdk.Network.GOERLI;
  }
...
class FaceWalletConnector extends BaseConnector {
  constructor({ logger, chains, testnetApiKey, mainnetApiKey }) {
    ...
    this.resolveApiKey = (network) => {
      switch (network) {
        case faceSdk.Network.ETHEREUM:
        case faceSdk.Network.POLYGON:
        case faceSdk.Network.BNB_SMART_CHAIN:
        case faceSdk.Network.KLAYTN:
        case faceSdk.Network.BORA: {
          if (!this._mainnetApiKey) {
            throw new Error("No API key for mainnet");
          }
          return this._mainnetApiKey;
        }
        case faceSdk.Network.GOERLI:
        case faceSdk.Network.MUMBAI:
        case faceSdk.Network.BNB_SMART_CHAIN_TESTNET:
        case faceSdk.Network.BAOBAB:
        case faceSdk.Network.BORA_TESTNET: {
...
exports.FaceWalletConnector = FaceWalletConnector;
